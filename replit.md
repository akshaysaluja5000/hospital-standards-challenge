...
- **Study & Practice**: "Study Mode" with Anki-style spaced repetition flashcards. Card backs show keyPoint (direct answer) first prominently, then content (explanation) below. 4-button SR rating (Again <2 min, Hard <15 min, Good 1 day, Easy 3 days) persisted to `flashcard_reviews` DB table. Sessions re-queue Again/Hard cards immediately; Good/Easy schedule future review. Dashboard shows "Review Due Cards" banner with count badge when cards are overdue, linking to `/flashcard-review` cross-level review page. Card fronts show last persisted rating from DB. Keyboard shortcuts: Space=flip, →=Good. Category badges color-code each card as Rule/Definition/Scenario/Mistake/Number/Tip. Quiz sessions are persistent, allowing users to resume gameplay.
